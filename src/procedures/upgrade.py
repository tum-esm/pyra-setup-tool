from src import utils, routines


def run() -> None:
    local_pyra_versions = routines.find_versions.get_local_versions()
    remote_pyra_versions = routines.find_versions.get_remote_versions()
    ...
    routines.manage_local_files.download_version(version_to_be_installed)
    routines.installation.install_version(version_to_be_installed)

    available_versions_to_migrate_from = routines.find_versions.get_versions_to_migrate_from(
    ...
    if len(available_versions_to_migrate_from) == 0:
        ...
        if version_to_migrate_from != "no":
            routines.installation.migrate_config(
